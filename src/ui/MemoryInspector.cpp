...
namespace quartz::client
{
    namespace
    {
        ...
        bool requestAsyncDisassemblyFetch(RuntimeMemoryInspectorState& state, const std::uintptr_t focus, const std::optional<std::uintptr_t> scrollAnchor, const TextEditor::Scroll alignment)
        {
            ...
            {
                if (stop.stop_requested()) return; AsyncDisassemblyFetchResult result; result.Generation=generation; result.Pid=pid; result.Focus=focus; result.ReadSize=readSize; result.ScrollAnchor=scrollAnchor; result.Alignment=alignment;
        ...
        void drawPatchHistory(RuntimeMemoryInspectorState& state)
        {
            ...
            {
                auto& patch = ui.PatchHistory[i]; if (patch.Pid != state.Pid) continue; ImGui::PushID(reinterpret_cast<void*>(key.second));
            }
        }
