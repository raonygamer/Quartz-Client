...
namespace quartz::client::ui
{
    namespace
    {
        ...
        {
            StructExperimentDefinition definition; std::string error; state.EditorValid = runtimeEvaluateStructExperiment(state.Editor.GetText(),definition,error); state.EditorDiagnostic = state.EditorValid ? i18n::tr("re.definitionValid") : std::move(error); state.Editor.ClearSquiggles(1);
...
return Struct.define({
    ...
    owner: Field.Pointer(0x18),
            ...
            configureQuartzTypeScriptEditor(state.Editor,false,false); state.Editor.SetText(DefaultSource); state.AutoComplete = {}; state.AutoComplete.triggerDelay = std::chrono::milliseconds(70); state.AutoComplete.suggestionWidth = 36; state.AutoComplete.noSuggestionsLabel = i18n::tr("re.noMatchingFields");
            ...
            state.Editor.SetTextHoverCallback([&state](TextEditor::PopupData& data)
            {
                ...
                if (token == "Struct" || token == "Struct.define") { ImGui::TextUnformatted("Struct.define(fields: Record<string, Field>): Struct"); ImGui::Separator(); ImGui::TextWrapped("%s",i18n::tr("re.structDefineHelp")); return; }
                if (token == "Field") { ImGui::TextUnformatted("Quartz Field API"); ImGui::Separator(); ImGui::TextWrapped("%s",i18n::tr("re.fieldApiHelp")); return; }
    ...
    {
        static ObjectExperimentState state; initializeEditor(state); applyQuartzTextEditorPalette(state.Editor); ImGui::TextWrapped("%s",i18n::tr("re.objectDescription"));
        ...
        drawAddressInput(i18n::tr("re.baseAddress"),state.Address.data(),state.Address.size(),state.Pid,360.0f); ImGui::SameLine(); if (ImGui::Button(i18n::tr("re.bindAddress"))) { std::uintptr_t address=0; std::string error; if (state.Pid<=0) state.Status=i18n::tr("common.selectProcess"); else if (!evaluateAddressExpression(state.Pid,state.Address.data(),address,error)||address==0) state.Status=error.empty()?i18n::tr("re.invalidBaseAddress"):error; else { state.Base=address; state.LastRefresh=0.0; state.Status=i18n::tr("re.addressBound"); refreshRows(state); } }
        ImGui::SameLine(); if (ImGui::Button(i18n::tr("re.useInspectorAddress"))) { const auto& inspector=runtimeMemoryInspectorState(); if (inspector.Pid>0&&inspector.Address!=0) { state.Pid=inspector.Pid; setSharedReverseEngineeringProcess(state.Pid); state.Base=inspector.Address; std::snprintf(state.Address.data(),state.Address.size(),"0x%llX",static_cast<unsigned long long>(state.Base)); state.LastRefresh=0.0; state.Status=i18n::tr("re.usingInspectorAddress"); refreshRows(state); } else state.Status=i18n::tr("re.noInspectorAddress"); }
        ImGui::SetNextItemWidth(180.0f); ImGui::SliderFloat(i18n::tr("re.liveRefresh"),&state.RefreshHz,0.5f,60.0f,"%.1f Hz",ImGuiSliderFlags_Logarithmic); ImGui::SameLine(); ImGui::TextDisabled("%s",i18n::tr("re.addressExpressionHint"));
        ImGui::SeparatorText(i18n::tr("re.definition")); if (ImGui::Button(i18n::tr("re.applyDefinition"))) { StructExperimentDefinition definition; std::string error; if (runtimeEvaluateStructExperiment(state.Editor.GetText(),definition,error)) { state.Definition=std::move(definition); state.HasDefinition=true; state.LastRefresh=0.0; state.Status=std::to_string(state.Definition.Fields.size())+" "+i18n::tr("re.topLevelFieldsApplied"); refreshRows(state); } else { state.HasDefinition=false; state.Rows.clear(); state.Status=std::move(error); } }
        ImGui::SameLine(); if (ImGui::Button(i18n::tr("re.resetExample"))) { state.EditorInitialized=false; initializeEditor(state); } ImGui::SameLine(); ImGui::TextDisabled("%s",i18n::tr("re.objectEditorHint")); state.Editor.Render("##ObjectExperimentEditor",ImVec2(-1.0f,230.0f));
        ...
        if (!state.HasDefinition) { ImGui::TextDisabled("%s",i18n::tr("re.applyDefinitionHint")); return; } if (state.Pid<=0||state.Base==0) { ImGui::TextDisabled("%s",i18n::tr("re.bindObjectHint")); return; }
        const double now=runtimeSteadySeconds(); if (state.LastRefresh==0.0||now-state.LastRefresh>=1.0/std::max(static_cast<double>(state.RefreshHz),0.1)) refreshRows(state); const auto modules=enumerateRuntimeModules(state.Pid); ImGui::SeparatorText(i18n::tr("re.boundObject")); const std::string baseText=runtimeFormatProcessAddress(modules,state.Base); ImGui::TextUnformatted(baseText.c_str()); ImGui::SameLine(); ImGui::TextDisabled(i18n::tr("re.boundObjectStatus"),static_cast<int>(state.Pid),runtimeX86ModeName(runtimeProcessX86Mode(state.Pid)),state.Rows.size()); if (ImGui::Button(i18n::tr("re.inspectBase"))) openInspector(manager,state.Pid,state.Base); ImGui::SameLine(); if (ImGui::Button(i18n::tr("re.refreshNow"))) refreshRows(state);
        ...
        {
            ImGui::TableSetupColumn(i18n::tr("re.field")); ImGui::TableSetupColumn(i18n::tr("re.offset"),ImGuiTableColumnFlags_WidthFixed,88.0f); ImGui::TableSetupColumn(i18n::tr("re.address"),ImGuiTableColumnFlags_WidthFixed,190.0f); ImGui::TableSetupColumn(i18n::tr("re.type"),ImGuiTableColumnFlags_WidthFixed,105.0f); ImGui::TableSetupColumn(i18n::tr("re.value")); ImGui::TableSetupColumn(i18n::tr("re.raw")); ImGui::TableHeadersRow();
            for (std::size_t i=0;i<state.Rows.size();++i)
            {
                ...
                if (ImGui::BeginPopupContextItem("ObjectExperimentContext")) { if (ImGui::MenuItem(i18n::tr("re.inspectField"))) openInspector(manager,state.Pid,row.Address); ImGui::BeginDisabled(!row.WatchType||row.Width==0); if (ImGui::MenuItem(i18n::tr("re.addFieldWatch"))) addWatch(manager,state.Pid,row); if (ImGui::MenuItem(i18n::tr("re.watchFieldAccesses"))) openAccessWatch(manager,state.Pid,row.Address,row.Width); ImGui::EndDisabled(); ImGui::BeginDisabled(row.PointerValue==0); if (ImGui::MenuItem(i18n::tr("re.inspectPointer"))) openInspector(manager,state.Pid,row.PointerValue); ImGui::EndDisabled(); ImGui::Separator(); if (ImGui::MenuItem(i18n::tr("re.copyAddress"))) { const std::string text=runtimeFormatProcessAddress(modules,row.Address); ImGui::SetClipboardText(text.c_str()); } if (ImGui::MenuItem(i18n::tr("re.copyAbsoluteAddress"))) { const std::string text=runtimeHexAddress(row.Address); ImGui::SetClipboardText(text.c_str()); } if (ImGui::MenuItem(i18n::tr("re.copyValue"))) ImGui::SetClipboardText(row.Value.c_str()); ImGui::EndPopup(); }
